...
int login(int sock);
int signup(int sock);
int admin_menu(int sock, int id);
int user_menu(int sock, int id, int type);
...
int login(int sock){
	...
	lock.l_start = (acc_no-1)*sizeof(struct account);  //lock on customer record
	...
	if(type == 1){
		// customer
		lock.l_type = F_WRLCK;//customer can't login from multiple points simultaneously
		...
		if(temp.id == acc_no){
			if(!strcmp(temp.pass, password)){
				...
				while(-1!=user_menu(sock, temp.id, type));
				login_success = 1;
			}
		}
		lock.l_type = F_UNLCK;
		...
		//agent
		lock.l_type = F_RDLCK;  /////one agent can login multiple times
		fcntl(fd,F_SETLK, &lock);
		...
		if(ret != -1){
			...
			if(temp.id == acc_no){
				if(!strcmp(temp.pass, password)){
					write(sock, &valid, sizeof(valid));
					while(-1!=admin_menu(sock, temp.id));
					login_success = 1;
				}
			}
			lock.l_type = F_UNLCK;
...
int admin_menu(int sock, int id){
	...
	if(op_id == 1){
		...
		temp.tid = tid;
		temp.train_no = tno; //input no from socket
		strcpy(temp.train_name, tname); //input name from socket
		temp.av_seats = 15;
		...
		fcntl(fd, F_SETLKW, &lock); //lock whole train database file
	...
	if(op_id == 2){
		...
		read(sock, &no_of_trains, sizeof(int)); //here no_of_trains is interpreted as train id
		if(no_of_trains != -2)			//returns true when admin wants to delete a train
		{
			struct train temp;
			//lseek(fd, 0, SEEK_SET);
...
int user_menu(int sock, int id, int type){
	...
	if(op_id == 1){
		...
		fcntl(fd, F_SETLKW, &lock); //lock on whole train database
